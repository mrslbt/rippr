...
import { saveTranscript, listSavedTranscripts, readSavedTranscript, DEFAULT_SAVE_DIR, } from "./storage.js";
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "rip_transcript",
            description: "Extract the full transcript from a YouTube video. By default, saves the transcript as a Markdown file to disk and returns a resource_link + metadata (title, channel, language, duration, word count, saved path, preview). The full transcript text is NOT returned by default — this keeps context lean and gives the user a persistent file they can reuse. After calling, always tell the user where the file was saved. If you need the transcript text later (summarize, search, extract quotes), read the returned resource rather than re-ripping. Pass return_text: true only for short clips or when the user explicitly asks for the transcript inline. Pass format: 'segments' to save timestamped JSON instead of Markdown.",
            inputSchema: {
                type: "object",
                properties: {
                    url: {
                        type: "string",
                        description: "YouTube video URL. Supports any YouTube URL format (watch, youtu.be, embed, shorts, or bare 11-char ID).",
                    },
                    format: {
                        type: "string",
                        enum: ["text", "segments"],
                        description: "Saved file format. 'text' writes a Markdown file with YAML frontmatter and a continuous transcript block (best for RAG/LLM). 'segments' writes a JSON file with timestamped segments (best for chapter markers, precise citations). Default: text.",
                    },
                    save_path: {
                        type: "string",
                        description: "Optional override for where to save the transcript. Can be an absolute path, a ~/-relative path, a directory (file is named automatically), or a full file path. Default: ~/rippr/transcripts/<slug>_<videoId>.<ext>",
                    },
                    return_text: {
                        type: "boolean",
                        description: "If true, include the full transcript text in the tool response alongside the resource_link. Default: false. Use true only for short clips or when the user explicitly wants the transcript inline.",
                    },
                },
                required: ["url"],
            },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        const duration = result.segments.reduce((acc, s) => Math.max(acc, s.start + s.duration), 0);
        const wordCount = fullText.split(/\s+/).filter(Boolean).length;
        const previewSource = fullText.replace(/\s+/g, " ").trim();
        const preview = previewSource.slice(0, 240) + (previewSource.length > 240 ? "…" : "");
        const fileUri = pathToFileURL(saved.path).href;
        const mimeType = format === "segments" ? "application/json" : "text/markdown";
        const summaryText = [
...
server.setRequestHandler(ListPromptsRequestSchema, async () => ({
    prompts: [
        ...
        {
            name: "extract_quotes",
            description: "Rip a YouTube video and pull notable quotes or key statements.",
            arguments: [
                { name: "url", description: "YouTube video URL", required: true },
                {
                    name: "topic",
        ...
        {
            name: "research_topic",
            description: "Rip a YouTube video and extract information relevant to a specific topic.",
            arguments: [
                { name: "url", description: "YouTube video URL", required: true },
            ...
            contents: [
                {
                    ...
                    text: JSON.stringify({
                        formats: [
                            {
                                name: "text",
                                description: "Saved as Markdown with YAML frontmatter. Best for LLM consumption, RAG pipelines, summarization, and human reading.",
                                default: true,
                            },
                            {
                                name: "segments",
                                description: "Saved as JSON with timestamped segments. Best for referencing specific moments, building chapter markers, or precise citations.",
                                default: false,
                            },
                        ],
...
async function main() {
    ...
    console.error(`rippr-mcp server running on stdio (transcripts → ${DEFAULT_SAVE_DIR})`);
}
main().catch((error) => {
